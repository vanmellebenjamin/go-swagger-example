...
func configureAPI(api *operations.TodoListAPI) http.Handler {
	...
	api.TodosAddOneHandler = todos.AddOneHandlerFunc(func(params todos.AddOneParams) middleware.Responder {
		...
		if err != nil && err.Error() == "already_exist" {
			return middleware.Error(http.StatusConflict, nil)
		} else if err != nil {
	...
	api.TodosDestroyOneHandler = todos.DestroyOneHandlerFunc(func(params todos.DestroyOneParams) middleware.Responder {
		itemRepository.DeleteItem(params.ID)
	...
	api.TodosGetOneHandler = todos.GetOneHandlerFunc(func(params todos.GetOneParams) middleware.Responder {
		...
		if err != nil && err.Error() == "not_found" {
			return middleware.Error(http.StatusNotFound, nil)
		} else if err != nil {
			return middleware.Error(http.StatusInternalServerError, nil)
		} else {
	...
	api.TodosUpdateOneHandler = todos.UpdateOneHandlerFunc(func(params todos.UpdateOneParams) middleware.Responder {
		...
		if err != nil && err.Error() == "not_found" {
			...
			return todos.NewAddOneCreated().WithPayload(itemResult)
		}
	})
